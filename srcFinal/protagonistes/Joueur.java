...
public class Joueur extends Combattant {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4522714498279583581L;
...
//	public Joueur(String nom) { //constructeur pour le md 
//		super(nom, nbVieJoueur); //par defaut le joueur a 10 nb de vie
//		//par defaut le joueur commence dans la piece (1,1) qui est a l'indexe 0 de la liste des pieces
//		this.localisationJoueurListe = 0;
...
//	}	
	...
	public String seDeplacer(Direction direction)
	{
		if(direction == Direction.NORD)
		{
			...
			else if(!this.localisationJoueur.getPorteNord().isEstActive())
				return "Cette porte est bloquee!";
			else
		...
		if(direction == Direction.SUD)
		{
			...
			else if(!this.localisationJoueur.getPorteSud().isEstActive())
				return "Cette porte est bloquee!\n";
			else
				this.localisationJoueur = Donjon.piecesDonjon.get(this.localisationJoueurListe = this.localisationJoueurListe - Donjon.dimension);
		}
		...
		if(direction == Direction.EST)
		{
			...
			else if(!this.localisationJoueur.getPorteEst().isEstActive())
				return "Cette porte est bloquee!\n";
			else
		...
		if(direction == Direction.OUEST)
		{
			...
			else if(!this.localisationJoueur.getPorteOuest().isEstActive())
				return "Cette porte est bloquee!";
			else
				this.localisationJoueur = Donjon.piecesDonjon.get(--this.localisationJoueurListe);
		}
		
		//si le joueur accede a une salle ou il n'ya plus de monstre mais le md a ajoute un tresor
		if(this.localisationJoueur.getMonstre() == null || this.localisationJoueur.getMonstre().getNbVie() < 0)
			System.out.println(this.sEquiper());
	...
	{
		if(this.localisationJoueur.getTresor() == null)
			return "La piece ne contient pas de tresors!\n";
		if(this.localisationJoueur.getTresor().getTypeTresor() == TypeTresor.ARME)
		{
			this.setArme((Arme) this.localisationJoueur.getTresor());
			return "Le courageux " + this.getNom() + " a trouve l'arme " + this.getArme().getNom() + Arme.emojiArme + "(+" + this.getArme().getDegats() +")" + " et va s'en equiper\n";
		}

		this.setArmure((Armure) this.localisationJoueur.getTresor());
		
		//enlever le tresor de la piece apres l'avoir equiper
		this.localisationJoueur.setTresor(null);
		return "Le courageux " + this.getNom() + " a trouve l'armure " + this.getArmure().getNom() + Armure.emojiArmure + "(x" + this.getArmure().getPointsProtection() +") " +"et va s'en equiper\n";
	}
